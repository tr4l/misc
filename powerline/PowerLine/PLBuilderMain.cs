    ...
    class PLBuilderMain
    {
        ...
        static bool writeFunctionFile(string functionsFile, List<string> remScripts, List<string> localScripts)
        {
            ...
            foreach (string script in localScripts)
            {
                ...
                Console.WriteLine("\t... trying to load " + script + "\r\n");
                string folder = Path.GetDirectoryName(Process.GetCurrentProcess().MainModule.FileName);
                byte[] bytes = File.ReadAllBytes(Path.GetFullPath(Path.Combine(folder, script)));
                //XOR "encrypt"
                ...
                Console.WriteLine("\tExample: powerline.exe " + scriptName.Split('.')[0] + " \"<command> | Out-Default\"\r\n");
